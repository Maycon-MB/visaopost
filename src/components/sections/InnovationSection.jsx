      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: '30px' }}>
        {innovations.map((inv, i) => (
          <motion.div key={i} whileHover={{ y: -10 }} style={{
            ...
            <h3 style={{ fontSize: '1.3rem', fontWeight: '800', marginBottom: '16px', color: '#1A2E1C' }}>{inv.title}</h3>
            <p style={{ color: 'rgba(0,0,0,0.6)', lineHeight: 1.7 }}>{inv.desc}</p>
            {inv.badge && (
              <span style={{
                position: 'absolute', top: '24px', right: '30px',
                fontSize: '0.65rem', textTransform: 'uppercase', letterSpacing: '1px',
                backgroundColor: 'rgba(212,136,10,0.1)', padding: '6px 12px',
