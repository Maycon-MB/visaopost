      ...
      return [
        {
          ...
          desc: 'Monitoramos sua conexão 24h para garantir que sua vitrine digital esteja always ativa e protegida contra quedas ou bloqueios.'
        },
        ...common,
        {
          icon: <Zap size={24} color={colors.gold} />,
  ...
  return (
    <AnimatePresence>
      <div style={{
        ...
        padding: '20px', backgroundColor: 'rgba(0,0,0,0.85)', backdropFilter: 'blur(10px)'
      }}>
        <motion.div
        ...
        >
          <button onClick={onClose} style={{ position: 'absolute', top: '24px', right: '24px', background: 'rgba(255,255,255,0.05)', border: 'none', color: 'white', cursor: 'pointer', width: '40px', height: '40px', borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
            <X size={20} />
          </button>

          <div style={{ textAlign: 'center', marginBottom: '40px' }}>
            <div style={{ display: 'inline-flex', padding: '12px', borderRadius: '50%', backgroundColor: 'rgba(212, 136, 10, 0.15)', marginBottom: '16px' }}>
              <ShieldCheck size={32} color={colors.gold} />
            </div>
            <h2 style={{ fontSize: '2rem', fontFamily: fonts.heading, color: 'white', marginBottom: '12px' }}>
              O Valor da sua Assinatura
            </h2>
            <p style={{ color: 'rgba(255,255,255,0.5)', fontSize: '1rem' }}>Por que o plano <strong>{planName}</strong> garante o futuro da sua ótica.</p>
          </div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(240px, 1fr))', gap: '30px' }}>
            {items.map((item, idx) => (
              <div key={idx} style={{ display: 'flex', gap: '15px' }}>
                ...
                <div>
                  <h4 style={{ color: 'white', fontWeight: '800', marginBottom: '8px', fontSize: '1rem' }}>{item.title}</h4>
                  <p style={{ color: 'rgba(255,255,255,0.6)', fontSize: '0.9rem', lineHeight: 1.6 }}>{item.desc}</p>
                </div>
              </div>
            ))}
          </div>
          ...
          <div style={{ marginTop: '40px', paddingTop: '30px', borderTop: '1px solid rgba(255,255,255,0.1)' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '20px', textAlign: 'left' }}>
              <div style={{ padding: '20px', background: 'rgba(255,255,255,0.02)', borderRadius: '16px' }}>
                <p style={{ color: 'rgba(255,255,255,0.4)', fontSize: '0.8rem', marginBottom: '6px' }}>Contratar Agência/Func</p>
                <p style={{ color: '#ff453a', fontWeight: '800', fontSize: '1.2rem' }}>R$ 2.500+ /mês</p>
                <p style={{ color: 'rgba(255,255,255,0.4)', fontSize: '0.75rem', marginTop: '4px' }}>+ Encargos e incerteza.</p>
              </div>
              <div style={{ padding: '20px', background: 'rgba(212, 136, 10, 0.1)', borderRadius: '16px', border: '1px solid rgba(212, 136, 10, 0.2)' }}>
                <p style={{ color: colors.gold, fontSize: '0.8rem', marginBottom: '6px' }}>Sua Máquina Projeto</p>
                <p style={{ color: '#32d74b', fontWeight: '800', fontSize: '1.2rem' }}>R$ {planName === 'Presença Digital' ? '97' : (planName === 'Vendas Ativas' ? '197' : '297')} /mês</p>
                <p style={{ color: 'rgba(255,255,255,0.5)', fontSize: '0.75rem', marginTop: '4px' }}>Tecnologia trabalhando 24/7.</p>
              </div>
            </div>
          </div>
        </motion.div>
